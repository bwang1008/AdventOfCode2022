...
let main () = 
        ...
        let add_neighbor (parent_row : int) (parent_col : int) (row : int) (col : int) (queue : (int * int) list) (ans : (int option) list list) : ((int * int) list * (int option) list list) = 
          ...
          | true ->
              ...
              | true -> 
                  ...
                  | Some x -> (queue, ans)
                  | None -> 
                      (* now check edges: can only go down at most 1, but can go up many *)
  ...
  in
  let distances : ((int option) list list) option = Option.map (fun ending_position -> 
    let () = Printf.printf "IN first bind. ending_position = (%d, %d)\n" (fst ending_position) (snd ending_position) in
    let empty_matrix : (int option) list list = List.init num_rows (fun col_index -> List.init num_cols (fun index -> None)) in
    let empty_matrix : (int option) list list = set_value_in_matrix empty_matrix (fst (Option.get end_position)) (snd (Option.get end_position)) 0 in
    let () = Printf.printf "Ok empty matrix size: %d x %d\n" (List.length empty_matrix) (List.length (List.hd empty_matrix)) in
    bfs [ending_position] empty_matrix
  ) end_position
  in
  let () = Printf.printf "Point D\n" in
  let ans : int option = Option.map (fun distances -> 
    let min_dist_to_a : int = 
      let height_map : int list = List.concat height_map in
      let distances : (int option) list = List.concat distances in
      let distances : int list = List.mapi (fun index distance ->
        match distance with
          | None -> num_rows * num_cols
          | Some distance ->
              match (List.nth height_map index) with
              | 0 -> distance
              | _ -> num_rows * num_cols
      ) distances
      in
      List.fold_left min (num_rows * num_cols) distances
    in
    min_dist_to_a
  ) distances
  in

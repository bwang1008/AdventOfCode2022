...
let main () = 
  ...
  in
  (* convert line of input, to two ranges, and see if they overlap *)
  let parse_line (input : string) : bool = 
    ...
    let disjoint_ranges (r1 : int * int) (r2 : int * int) : bool = 
      (snd r1) < (fst r2) || (snd r2) < (fst r1)
    in
    not (disjoint_ranges range1 range2)
  in
